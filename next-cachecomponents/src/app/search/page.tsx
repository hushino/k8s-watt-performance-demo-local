...
export default async function SearchPage({
  ...
  return (
    <Suspense fallback={<div className="py-8 text-gray-500">Loading search results...</div>}>
...
async function SearchPageContent({
  ...
  const [results, games] = await Promise.all([getCachedSearchResult(searchConfig), getCachedGames()]);
...
async function getCachedSearchResult(params: CardSearchParams) {
  'use cache';
  applyDailyCache(`page:search:${JSON.stringify(params)}`);
  return db.searchCards(params);
}

async function getCachedGames() {
  'use cache';
  applyDailyCache('page:search:games');
  return db.getGames();
}
