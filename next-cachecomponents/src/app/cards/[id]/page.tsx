...
export default function CardDetailPage({
  ...
  return (
    <Suspense fallback={<div className="py-8 text-gray-500">Loading card details...</div>}>
      <CardDetailPageContent params={params} />
    </Suspense>
  );
}

async function CardDetailPageContent({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  const card = await getCachedCardPayload(id);
  ...
  const [set, game, sellers] = await Promise.all([
    getCachedSetBySlug(card.setId.replace(`${card.gameId}-`, '').replace(/-set-\d+$/, '')),
    getCachedGameBySlug(card.gameId),
    getCachedSellers(),
  ]);
...
async function getCachedCardPayload(id: string) {
  'use cache';
  applyDailyCache(`page:card:${id}`);
  return db.getCardWithListings(id);
}

async function getCachedSetBySlug(slug: string) {
  'use cache';
  applyDailyCache(`page:card:set:${slug}`);
  return db.getSetBySlug(slug);
}

async function getCachedGameBySlug(slug: string) {
  'use cache';
  applyDailyCache(`page:card:game:${slug}`);
  return db.getGameBySlug(slug);
}

async function getCachedSellers() {
  'use cache';
  applyDailyCache('page:card:sellers');
