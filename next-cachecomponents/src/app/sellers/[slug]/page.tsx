...
export default async function SellerDetailPage({
  ...
  return (
    <Suspense fallback={<div className="py-8 text-gray-500">Loading seller details...</div>}>
      <SellerDetailPageContent params={params} searchParams={searchParams} />
    </Suspense>
  );
}

async function SellerDetailPageContent({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>;
  ...
  const seller = await getCachedSellerPayload(slug, page, limit);

  if (!seller) {
    notFound();
  }

  const cards = await getCachedCardsByIds(
    'cards',
...
async function getCachedSellerPayload(slug: string, page: number, limit: number) {
  'use cache';
  applyDailyCache(`page:seller:${slug}:${page}:${limit}`);
  return db.getSellerWithListings(slug, page, limit);
}

async function getCachedCardsByIds(collection: string, ids: string[]) {
  'use cache';
  applyDailyCache(`page:seller:cards:${collection}:${JSON.stringify(ids)}`);
  return db.findMany<{ id: string; name: string; number: string }>(collection, ids);
}
