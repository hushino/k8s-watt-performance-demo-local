...
export default async function GameDetailPage({
  ...
  return (
    <Suspense fallback={<div className="py-8 text-gray-500">Loading game details...</div>}>
      <GameDetailPageContent params={params} />
    </Suspense>
  );
}

async function GameDetailPageContent({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const game = await getCachedGamePayload(slug);
...
async function getCachedGamePayload(slug: string) {
  'use cache';
  applyDailyCache(`page:game:${slug}`);
