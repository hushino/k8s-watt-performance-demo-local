...
import { applyDailyCache } from './cache';
import type {
  Game,
  CardSet,
  Card,
...
async function readCollectionWithCache<T>(dataDir: string, name: string): Promise<T[]> {
  'use cache';

  applyDailyCache(`collection:${name}`);
...
async function readFeaturedWithCache(dataDir: string): Promise<Featured | null> {
  'use cache';

  applyDailyCache('collection:featured');
  ...
  try {
    const content = await fs.promises.readFile(filePath, 'utf-8');
    return JSON.parse(content) as Featured;
  } catch {
    return null;
  }
}
...
class JsonDatabase {
  ...
  constructor(dataDir?: string) {
    ...
    this.delayEnabled = process.env.DB_DELAY_ENABLED === 'true';
  }
  ...
  private async loadCollection<T>(name: string): Promise<T[]> {
    ...
    const data = await readCollectionWithCache<T>(this.dataDir, name);
  ...
  async getFeaturedContent(): Promise<Featured | null> {
    await this.delay();
    return readFeaturedWithCache(this.dataDir);
  }
